...
Если на компьютере установлен Microsoft Office и при установке LibreOffice были настроены ассоциации файлов Microsoft Office (подробнее смотрите *Приложени Б*) с LibreOffice, то при двойном нажатии на файл:
...
Вы можете открыть существующие документы следующими способами:
...
В диалоге открытия файлов вы можете сократить список отображаемых файлов, задав тип (расширение) необходимых вам файлов. Например, если вы укажете тип файла *Текстовый документ*, то увидите в окне только документы Writer (включая файлы с расширением ``.odt``, ``.doc``, ``.txt``); если укажите *Электронные таблицы*, то вы увидите файлы с расширениями ``.ods``, ``.xls`` и другие файлы, открываемые в Calc.

Также вы можете открыть существующий документ в формате, который поддерживает LibreOffice,  нажав два раза на файл на рабочем столе или в окне файлового менеджера (таком как Windows Explorer). Будет запущен соответствующий формату файла компонент LibreOffice.
...
* Используя команду :guilabel:`Сохранить` (нажмите на клавиатуре ``Ctrl + S`` или используйте пункт меню :menuselection:`&Файл --> Со&хранить` или нажмите на значок |lo-save| на стандартной панели инструментов). Если вы сохраняете документ с его текущим именем и месторасположением. Команда :guilabel:`Сохранить` перезапишет последний сохраненный вариант документа. Если документ сохраняется впервые, то будет выведено диалоговое окно сохранения документа, в котором можно будет выбрать название, формат и местоположение документа.

.. |lo-save| image:: _static/icons/lo-save-doc-icon.png
    :scale: 55%

* Используя команду :guilabel:`Сохранить как...` (нажмите на клавиатуре ``Ctrl + Shift + S`` или используйте пункт меню :menuselection:`&Файл --> Сохранить &как...`) если вы хотите создать новый документ из текущего или изменить имя файла и/или формат файла, или сохранить файл в другом месте.

Используйте защиту документа паролем, если вы хотите ограничить число людей, которые могут открывать, просматривать и редактировать документ, подробнее в разделе `Защита паролем`_.
...
Чтобы ограничить число людей, которые могут открывать и читать документ или открывать и редактировать документ, вы должны использовать защиту документа паролем, для этого:
...
6. Нажмите кнопку :guilabel:`OKё для закрытия диалога. Если пароли совпадают, то документ сохранится с защитой паролем. Если пароли не совпадают, то вы увидите сообщение об ошибке.
...
Вы можете выбрать, какие диалоги открытия/сохранения файлов использовать: те, которые есть в самом LibreOffice, или стандартные диалоги вашей операционной системы.
...
Чтобы открыть :guilabel:`Навигатор`, нажмите на значок :guilabel:`Навигатора` |lo-navigator| на стандартной панели инструментов или нажмите клавишу ``F5`` на клавиатуре, или откройте меню :menuselection:`&Вид --> Навига&тор`. Также вы можете нажать на область в :guilabel:`Строке` состояния, показывающую количество страниц, подробнее смотрите раздел `Строка состояния`_.

.. |lo-navigator| image:: _static/icons/lo-navigator-icon.png
                  :scale: 60%

Вы можете закрепить :guilabel:`Навигатор` по обе стороны главного окна программы или оставить его плавающим. нажмите на маркер (``+`` или треугольник) рядом с любой категорией, чтобы отобразить список объектов в этой категории.
...
* Объекты легче искать, если им присваивать при создании узнаваемые имена, а не использовать стандартные имена LibreOffice типа Лист1, Таблица1, Таблица2 и тому подобные. Стандартное имя может не соответствовать фактическому положению объекта в документе.
...
Чтобы отменить самое последнее изменение в документе используйте сочетание клавиш ``Ctrl+Z`` или нажмите на значок :guilabel:`Отменить` |lo-undo| на стандартной панели инструментов, или используйте пункт главного меню :menuselection:`Правка --> Отменить`. Нажмите на маленький треугольник справа от значка :guilabel:`Отменить`, появится список всех изменений, которые можно отменить. Вы можете выбрать несколько изменений и отменить их все одновременно.
