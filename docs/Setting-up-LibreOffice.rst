...
* *Масштаб* — 
Если текст в файлах справки или меню интерфейса LibreOffice слишком маленький или слишком большой,  то вы можете изменить его размер, задав коэффициент масштаба (считая, что 100% - это нормально, меньше 100% - уменьшение, больше 100% увеличение). Иногда изменение этого параметра может иметь неожиданные последствия в зависимости от шрифтов, установленных в вашей системе. Но этот параметр не влияет на размер текста в вашем документе.

** Размер и стиль значков* — 

Первый параметр определяет размер значков на панелях инструментов (Автоматически, Маленькие или Большие). Значение Автоматически использует размер значков, установленный в настройках вашей операционной системы. Второй параметр определяет набор (тему) значков. Здесь значение Автоматически использует набор значков, совместимый с операционной системой и настройками рабочего стола, например KDE или Gnome в Linux.


* *Системный шрифт для интерфейса*

Если вы предпочитаете использовать в меню системные шрифты (шрифты, используемые в меню вашей операционной системы) вместо шрифтов, предлагаемых LibreOffice для интерфейса, то поставьте флажок у этого пункта.

* *Сглаживание экранных шрифтов* — 
(Недоступно в Windows; не показано на рисунке 5.) Активируйте этот пункт для сглаживания начертания шрифтов текста. Введите наименьший размер шрифта, начиная с которого шрифты будут сглаживаться.

**Меню**

 – значки в меню
Настройка отображения значков в пунктах меню. Может принимать значение Автоматически, Показать и Скрыть.


**Список шрифтов**

 - Предварительный просмотр шрифтов
Если флажок установлен, то список шрифтов выглядит, как на рисунке 6 слева, каждый шрифт отображается так, как он будет выглядеть в тексте. Если флажок снят, то список шрифтов будет выглядеть, как на рисунке 6 справа, весь список показан системным шрифтом.
