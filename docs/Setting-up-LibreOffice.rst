...
Параметры безопасности и предупреждения
Доступны следующие опции на Параметры безопасности и предупреждений диалога (рисунок 23).
Удалить персональную информацию при сохранении
Выберите эту опцию, чтобы всегда удалить данные пользователя из свойств файла при сохранении файла. для
вручную удалить личную информацию из определенных документов, снимите этот флажок.
Ctrl клик обязаны следовать гиперссылки
Поведение по умолчанию в LibreOffice является Ctrl + щелчок на гиперссылке, чтобы открыть связанный документ.
Многие люди считают, создание и редактирование документов легче, когда случайные клики по ссылкам делать
Не включайте ссылки. Чтобы установить LibreOffice, чтобы активировать гиперссылки, используя обычный щелчок, снимите
Эта опция.
Другие варианты в этом диалоговом окне должны быть понятны.
...
You can enter a master password to enable easy access to websites that require a user name
and password. If you select the Persistently save passwords for web connections option, a
new dialog opens (Figure 22). LibreOffice will securely store all passwords that you use to
access files from web servers. You can retrieve the passwords from the list after you enter the
master password.
